...
def setup_main_logger():
    ...
    return Logger.getLogger('wlscli')

def setup_command_logger():
    return Logger.getLogger('wlscli' + command_name)
...
def usage():
    main_logger.info('Usage: wlscli <command> [<environment> <config_file>]\n')    
...
def print_licence_info():	
    main_logger.info("===================================================================================================")
#    main_logger.info("WL COE PLATFORM v1.0")
    main_logger.info("wlscli help to see availible commands")
#    main_logger.info("If you are having any query to use WL COE PLATFORM, please get in touch with us via slack channel.")
    main_logger.info("===================================================================================================")
...
def check_java_version():
	...
	if minor_version < 5 or minor_version > 8:
		...
		main_logger.warn("Java version " + java_version + " is not supported. Commands may not function as intended")
		time.sleep(5)
...
try:
    ...
    if customLog is not None and len(customLog) > 0:
        if customLog == 'true':
            customLog = 'wlscli_' + command_name + "_" + datetime.now().strftime("%Y%m%d_%H%M%S.log")
    ...
    if command_file:
        ...
        if is_jython(command_file):
            ...
            if 'run' in dir():
                ...
                if run_arg_count == 0:
                    if arg_count == 1:
                        ...
                        usage()
                        main_logger.info('e.g.: wlscli ' + command_name)                        
                        sys.exit()
                    ...
                    if rb_config:                        
                        ...
                        main_logger.info('e.g.: wlscli ' + command_name + ' example simple')                        
                        sys.exit()
                else:
            ...
            if rb_config.size()<=0:                        
                ...
                usage()
                main_logger.error('Command \'' + command_name + '\' requires <environment> <config_file> arguments')
                main_logger.info('e.g.: wlscli ' + command_name + ' local simple')                        
                sys.exit()
