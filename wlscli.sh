...
function set_env {
    ...
    if [ ${#relevantBits} -eq 0 ]; then
       ...
       java -classpath "${redback_cp}":${CLASSPATH}  -Duser.timezone='UTC' $ARGS -Dweblogic.management.confirmKeyfileCreation=true weblogic.WLST -skipWLSModuleScanning core/engine/main.py ${args[0]} ${args[1]} ${args[2]} ${args[3]} ${args[4]} ${args[5]} ${args[6]} ${args[7]} ${args[8]}
       if [ "$?" -eq 3 ]; then
...
function run_jython {
    ...
    java -Xms1g -Xmx1g -classpath "${redback_cp}":"${CLASSPATH}"  -Duser.timezone='UTC' -Dpython.home=. org.python.util.jython core/engine/main.py ${args[0]} ${args[1]} ${args[2]} ${args[3]} ${args[4]} ${args[5]} ${args[6]} ${args[7]} ${args[8]}
...
if test -z "$1"
then
    echo 'no arguments specified. please try   ./wlscli.sh help    for available commands'
else
    args=("$@")
fi
