...
def _getOsType( os=None ):
    ...
    _dosTypeMap = ( ("dos", ( 'dos', 'Windows 95', 'Windows 98', 'Windows ME' )),)
    ...
    if os.startswith(_windowsPattern):
        # Windows, either Dos type or nt type
        # Check Dos type first. The rest will be nt type.
        for dosType, patterns in _dosTypeMap:
            for pattern in patterns:
                if os.startswith(pattern):
                  foundType = dosType
                  break;
            # end of for pattern in patterns
            if foundType:
                # DOS type
                break;
        # end of for dosType, patterns in _dosTypeMap
        if not foundType:
            # nt type
            foundType = ntType
    else:
        # non-Windows, could be Mac type or posix type
        for osType, patterns in _nonWindowsTypeMap:
            for pattern in patterns:
                if os.startswith(pattern):
                  foundType = osType
                  break;
            if foundType:
                break;
        # end of for osType, patterns in _nonWndowsTypeMap

    if not foundType:
        # Not Windows type, not mac type, then it's posix type (e.g. Linux, Unix, Solaris, etc.)
